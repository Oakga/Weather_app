...
const createWeeklyReportTable = (data) => {
    ...
    addDropDownMenu(weeklyReport);
    updateChart(series,"Humidity");
};

const addDropDownMenu = (data) => {
    const typeArray = [];
    for (const key of Object.keys(data[0])) {
        typeArray.push(key);
    }

    const defaultKey = typeArray[0];
    let dropDownList = ``;
    for( const element in typeArray){
        dropDownList+= `<li>${element}</li>`
    }
    let dropDownMenuOutput =`
        <div class="dropdown">
        <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">${defaultKey}</button>
    ...
    document.getElementById('dropDownMenu-component').innerHTML = dropDownMenuOutput;
}
