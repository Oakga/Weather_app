const weeklyReport = (Time,Humidity,Precipitation,Intensity,Wind,MaxTemperature,MinTemperature) => {
    this.Time = Time;
...
const todayReport = (Time,Summary,Humidity,Temperature, Precipitation,Intensity,Wind,) => {
    this.Time = Time;
...
let darkSky = (lat, lng, date = Date.now()) => {
    ...
    axios.get(apiCall,{
        ...
        for(let i=0;i<dailyDate.length;i++){
            ...
            const dayReport = weeklyReport[i] = new weeklyReport(
                formattedTime,
